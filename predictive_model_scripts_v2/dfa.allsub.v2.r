...
## Function for all subsets DFA - version 2;
#See dfa.allsub.2.help.doc;
# function output is changed to a list containing bestmods and a vector of null model stats;

#function definition;           
dfa.allsub.v2<-function(bug.cal,bug.vld,pred.cal,pred.vld,grps,candvar,numkeep,Pc) {
#####################;
## begin function ops;
...
#sorting complete;
print('Model screening complete. Computing O/E stats for subset of best models....',quote=F);
######################################;
...
#compute and print null model results;
   ...
   OE.cal<-apply(bug.cal[,nulltax],1,sum)/Enull; #vector of null-model O/E at calibration sites;
   OE.vld<-apply(bug.vld[,nulltax],1,sum)/Enull; #vector of null-model O/E at validation sites;
  ...
  #VERSION 2 -- vector of null stats;
...
subset.stats<-data.frame(order=rep(NA,totmod),F.stat=rep(NA,totmod), Wilks=rep(NA,totmod), 
     ...
     MNOE.vld=rep(NA,totmod),SDOE.vld=rep(NA,totmod),RMSE.vld=rep(NA,totmod),model=rep(NA,totmod));
...
for(mord in 1:maxord) {
 ...
 return(list(subset.stats=subset.stats,null.stats=nullstat)); #output is model summary data frame and vector of null stats;
